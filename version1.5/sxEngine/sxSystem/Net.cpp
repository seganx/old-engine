...
SEGAN_INLINE bool net_con_connected( Connection* con, NetMessage* netmsg )
{
	...
	case NPT_ACK:
		...
		{
			sxLog::Log( L"Net : error(disconnected) : can't find the requested message in the sent list " );
			con->Disconnect();
			return false;
		}
		return true;
...
bool Socket::Open( const word port )
{
	...
	{
		sxLog::Log( L"Error: Can't initialize socket. error code : %s !" , net_error_string( WSAGetLastError() ) );
		return false;
	}
	...
	{
		sxLog::Log( L"Error: Unable to make socket broadcast! error code : %s !" , net_error_string( WSAGetLastError() ) );
		Close();
		return false;
	}
	...
	{
		sxLog::Log( L"Error: Unable to bind socket! error code : %s !" , net_error_string( WSAGetLastError() ) );
		Close();
		return false;
	}
	...
	{
		sxLog::Log( L"Error: Unable to make non-blocking socket! error code : %s !" , net_error_string( WSAGetLastError() ) );
		Close();
		return false;
	}
	...
	default:
		{
			...
			if ( sndcount )
			{
				if ( sndcount > NET_MAX_SENDING_LIST + 5 )
					sxLog::Log( L"NET : warning : try to send too much data in the single pass ! : %d", sndcount );
...
//	network functions
//////////////////////////////////////////////////////////////////////////
SEGAN_API bool sx_net_initialize( const dword netID )
{
	sx_callstack();

	if ( s_netInternal  )
	{
		sxLog::Log( L"Warning: calling sx_net_initialize() failed du to network system was initialized!" );
		return 0;
	}
	...
	{
		sxLog::Log( L"Error: Network initialization on Windows failed! error code : %s !" , net_error_string( WSAGetLastError() ) );
	...
	{
		sxLog::Log( L"Error: Network initialization on Windows failed! Invalid version detected!" );
		netInitialized = false;
	}
	...
	{
		sxLog::Log( L"Error: function ::gethostname() failed with error code : %s !" , net_error_string( WSAGetLastError() ) );
		netInitialized = false;
	}
	...
	{
		sxLog::Log( L"Error: function ::gethostbyname() failed with error code : %s !" , net_error_string( WSAGetLastError() ) );
		netInitialized = false;
	}
	...
	if ( netInitialized )
	{
		...
		sxLog::Log(  L"Network system initialized successfully on Windows." );
		sxLog::Log_( L"    Name:		%s \r\n" , s_netInternal->name );
		sxLog::Log_( L"    IP:			%d.%d.%d.%d \r\n\r\n" , 
			s_netInternal->address.ip[0], s_netInternal->address.ip[1], s_netInternal->address.ip[2], s_netInternal->address.ip[3] );
	}
	else
	{
		sxLog::Log( L"The network system is now disabled." );
		WSACleanup();
	}
...
void sx_net_finalize( void )
{
	...
	if ( s_netInternal )
	{
		...
		WSACleanup();
		sxLog::Log( L"Network system Finalized.  \r\n" );
	}
	else
	{
		sxLog::Log( L"Warning: calling sx_net_finalize() failed du to network system was finalized or was not initialized!" );
	}
}
