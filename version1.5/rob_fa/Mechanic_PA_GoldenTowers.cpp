...
namespace GM
{
	...
	void Mechanic_PA_GoldenTowers::Update( float elpsTime )
	{
		...
		{
			//	title
			GameString* title = g_game->m_strings->Get( 579 );
			GameString* desc = g_game->m_strings->Get( 580 );
			if ( title && desc )
			{
				swprintf_s( title->text, 512, title->base, (m_index + 1) );
				swprintf_s( desc->text, 512, desc->base, m_cost, g_game->m_player->m_gold );

				m_panelEx->SetHint( L"579\n580" );
				m_progBar->SetHint( L"579\n580" );
			}
		}
	...
	void Mechanic_PA_GoldenTowers::OnGUIClick( sx::gui::PControl Sender )
	{
		...
		if ( m_time >= m_coolTime && g_game->m_player->m_gold >= m_cost )
		{
			...
			msg_SoundPlay msg( false, 0, 0, L"powerAttack", 5 );
