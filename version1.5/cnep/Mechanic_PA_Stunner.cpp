...
namespace GM
{
	...
	{
		sx_callstack_push(Mechanic_PA_Stunner::Mechanic_PA_Stunner());
	...
	{
		sx_callstack_push(Mechanic_PA_Stunner::Initialize());
	...
	{
		sx_callstack_push(Mechanic_PA_Stunner::Finalize());
	...
	void Mechanic_PA_Stunner::ProcessInput( bool& inputHandled, float elpsTime )
	{
		if ( NotInGame() || g_game->m_mouseMode == MS_CreateTower )
			return;
		sx_callstack_push(Mechanic_PA_Stunner::ProcessInput());
	...
	void Mechanic_PA_Stunner::Update( float elpsTime )
	{
		if ( !g_game->m_game_currentLevel || g_game->m_game_paused )
			return;
		sx_callstack_push(Mechanic_PA_Stunner::Update());
	...
	{
		sx_callstack_push(Mechanic_PA_Stunner::MsgProc(recieverID=%d, msg=%d), recieverID, msg);
	...
	{
		sx_callstack_push(Mechanic_PA_Stunner::OnGUIClick());
