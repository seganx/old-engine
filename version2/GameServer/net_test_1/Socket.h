...
public:
	...
	bool Send( const struct NetAddress& destination, const void* buffer, const int size );

	//! pick up data on the port and fill out address of sender
	sint Receive( const void* buffer, const int size, struct NetAddress* OUT sender );
