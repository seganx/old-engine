...
void uiControl::set_parent( uiControl* parent )
{
...
void uiControl::set_size( const float width, const float height )
{
...
void uiControl::update( float elpsTime, const matrix& viewInverse, const matrix& viewProjection, const uint vpwidth, const uint vpheight )
{
	...
	//  start building matrix
	{
		...
		//  apply center
		mat.set_translation( - state->center.x * m_size.x, - state->center.y * m_size.y, state->center.z );
	...
	if ( !inputReport->mouseLocked )
	{
		...
		if ( m_option & SX_GUI_CLIPCHILDS )
		{
			...
			if ( captured )
			{
				...
				{
					m_child[i]->process_input( inputReport );
				}
			}
			else
			{
