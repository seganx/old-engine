...
d3dDevice::d3dDevice( void )
{
	...
	ZeroMemory( &m_driverDisplayMode,	sizeof(m_driverDisplayMode)	);
	ZeroMemory( &m_debugInfo,			sizeof(m_debugInfo)			);
	ZeroMemory( &m_viewport,			sizeof(m_viewport)			);

	ZeroMemory( &m_creationData,		sizeof(m_creationData)		);
...
void d3dDevice::Initialize( const struct HWND__* displayHandle )
{
	...
	//	collect driver information
	{
		...
		for ( int i=0; i<256 && strVendor[i] ; i++ )
...
void d3dDevice::SetTexture( const d3dTexture_gl* texture, uint stage /*= 0 */ )
{
	...
	m_textureBuffer[stage].current = texture->m_tbo;
