...
#define MAX_NUM_BUFFER				512		//	number of connection buffer in the stack
#define MAX_QUEUE_BUFFER			512		//	maximum number of buffer in the queue
#define SIZEOF_NETMESSAGE			512		//	size of connection buffer
#define SIZEOF_NETMESSAGE_HEADER	8		//	size of header of connection buffer
#define SERVER_FLAG_TIME			1200	//	time period that server notify all client to see the flag by sending broadcast message
#define CONNECTION_ALIVE_TIME		1000	//	keep connection alive
#define CONNECT_REQUEST_TIME		500		//	time period to send request message to connect
...
{
	//	header
	word		id;						//	id of network system to avoid conflict with other network system
...
int net_compare_ack( const PNetMessage& cb1, const PNetMessage& cb2 );
	...
	case CONNECTED:
		{
			...
			m_socket->Send( m_destination, (char*)&netmsg, SIZEOF_NETMESSAGE_HEADER );
		}
		break;
	}
...
SEGAN_INLINE void Connection::Update( struct NetMessage* buffer, const float elpsTime, const float delayTime, const float timeOut )
{
	...
	case LISTENING:
		...
		if ( buffer->bytsRecved && buffer->id == s_netInternal->id && buffer->type == NMT_CONNECT )
		{
			...
			m_socket->Send( m_destination, (char*)&netmsg, SIZEOF_NETMESSAGE_HEADER + net_copy_name( &netmsg, m_name ) );
	...
	case CONNECTING:
		...
		if ( buffer->bytsRecved && buffer->id == s_netInternal->id && net_check_address( &buffer->address, &m_destination ) && buffer->type == NMT_WELCOME )
		{
			...
			m_socket->Send( m_destination, (char*)&msg, SIZEOF_NETMESSAGE_HEADER );
		...
		m_sendTime += elpsTime;
		if ( m_sendTime > CONNECT_REQUEST_TIME )
		{
			m_sendTime = 0;
			...
			case NMT_ALIVE:
				...
				if ( m_recAck == buffer->ack )
				{
					...
					while ( m_sendQueue.Count() )
					{
						...
						if ( m_sentList.Count() == MAX_QUEUE_BUFFER )
						{
							sint h = MAX_QUEUE_BUFFER - 1;
				...
				else if ( m_recAck < buffer->ack )
				{
					if ( buffer->subtype == NMT_USER )
					{
						...
						if ( m_msgList.Count() < MAX_QUEUE_BUFFER )
						{
					...
					m_socket->Send( m_destination, (char*)&lostmsg, SIZEOF_NETMESSAGE_HEADER );
...
bool Connection::Send( const char* buffer, const int size )
{
	...
	if ( !buffer || size<1 || size>512 ) return false;
	...
	if ( s_netInternal->msgPool.Count() )
	{
		...
		msg->size = SIZEOF_NETMESSAGE_HEADER + size;
...
, m_flagTime(0)
{
	ZeroMemory( &m_broadCastAddress, sizeof(m_broadCastAddress) );
}
...
bool Server::Start( const word port, const uint maxClients, CB_Server callback )
{
...
void Server::Stop( void )
{
	...
	if ( s_netInternal )
	{
		...
		m_socket.Send( m_broadCastAddress, (char*)&buffer, SIZEOF_NETMESSAGE_HEADER + net_copy_name( &buffer, m_name ) );
	}
...
void Server::Listen( const word clientPort )
{
	switch ( m_state )
	{
	case STOPPED:
	...
	m_broadCastAddress.port = clientPort;
	for ( int i=0; i<m_clients.Count(); i++ )
		m_clients[i]->Listen();
...
void Server::Update( const float elpsTime, const float delayTime, const float timeOut )
{
	...
	case LISTENING:
		...
		m_flagTime += elpsTime;
		if ( m_flagTime > SERVER_FLAG_TIME )
		{
			m_flagTime = 0;
	...
	case RUNNING:
		{
			NetMessage buffer;
			buffer.bytsRecved = m_socket.Receive( (char*)&buffer, SIZEOF_NETMESSAGE, &buffer.address );
...
// 	return m_socket.Send( m_broadCastAddress, (char*)&netmsg, SIZEOF_NETMESSAGE_HEADER + size );
}
...
{
	sx_callstack_push(sx_net_initialize());
...
{
	sx_callstack_push(sx_net_finalize());
...
sint net_copy_name( NetMessage* msg, const wchar* name )
{
	if ( name )
	{
		sint len = ( (sint)wcslen( name ) + 1 ) * 2;
		memcpy( msg->data, name, len );
		return len;
	}
	else
	{
		sint len = sizeof(s_netInternal->name);
		memcpy( msg->data, s_netInternal->name, len );
		return len;
	}
}
